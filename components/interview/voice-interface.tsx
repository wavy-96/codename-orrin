...
export function VoiceInterface({ 
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (remaining === 0) {
        ...
        // End interview automatically and show completion modal
        handleEndInterview(true).then(() => {
  ...
  const handleEndInterview = async (skipConfirmation = false) => {
    ...
    try {
      ...
      // Only redirect if not showing modal (manual end)
      if (!skipConfirmation) {
  ...
  return (
    <div className="flex flex-col h-full max-w-4xl mx-auto p-2 sm:p-4 space-y-4 relative">
      ...
      <Dialog open={showCompletionModal} onOpenChange={() => {}}>
        <DialogContent className="sm:max-w-lg border-0 shadow-2xl" showCloseButton={false}>
          <DialogHeader className="text-center space-y-6 pt-6">
            <div className="mx-auto w-24 h-24 bg-gradient-to-br from-emerald-400 via-blue-500 to-purple-500 rounded-full flex items-center justify-center shadow-lg animate-in zoom-in-50 duration-300">
            ...
            <DialogDescription className="text-lg text-muted-foreground max-w-sm mx-auto leading-relaxed">
              You've completed your interview session. Let's see how you performed!
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="flex flex-col gap-4 mt-6 pb-6">
            <Button 
              size="lg" 
              className="w-full h-14 text-lg font-semibold bg-gradient-to-r from-emerald-500 to-blue-600 hover:from-emerald-600 hover:to-blue-700 shadow-lg hover:shadow-xl transition-all"
              onClick={handleViewResults}
            >
              <TrendingUp className="mr-2 h-6 w-6" />
              View Your Score & Analytics
            </Button>
            
            <div className="flex items-center justify-center gap-2 text-sm text-muted-foreground">
              <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
              <p>Your performance has been evaluated and is ready for review</p>
            </div>
          </div>
        </DialogContent>
      </Dialog>
